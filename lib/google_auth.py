...
def save_user_info():
    ...
    user = User()
    existing_user = User.collection.get(f'users/{user_id}')
    if existing_user is None:
        user.id = user_info.get('id', None)
        user.email = user_info.get('email', None)
        user.name = user_info.get('name', None)
        user.profile_pic = user_info.get('picture', None)
        user.last_login = datetime.today()
        user.save()
        flask.session['user_email'] = user.email
    else:
        existing_user.last_login = datetime.today()
        existing_user.update()
        # update and store user's detail in session
        flask.session['user_email'] = existing_user.email
        record = GhostRecord.get_email_record(existing_user.email)
        flask.session['ghost_name'] = [r.name for r in record if r.name]
